...
void Client::DrawHUD() {
	// get taps.
	int headtaps = g_shots.headshots;

	// get round trip time in milliseconds.
	int ms = g_csgo.m_engine->IsInGame() ? std::max(0, (int)std::round(g_cl.m_latency * 1000.f)) : 0;

	// get tickrate.
	int rate = g_csgo.m_engine->IsInGame() ? (int)std::round(1.f / g_csgo.m_globals->m_interval) : 0;

	std::string text = tfm::format(XOR("Essential | %s | %ims | Taps: %s"), g_cl.m_user, ms, headtaps);
...
void Client::DoMove() {
	...
	m_original_angles = m_cmd->m_view_angles;
	...
	// disable conditions.
	if (g_csgo.m_gamerules->m_bFreezePeriod() || (g_cl.m_flags & FL_FROZEN) || !g_cl.m_processing)
		return;
...
void Client::EndMove(CUserCmd* cmd) {
	...
	if (g_cl.m_lag <= 0) {
		...
		if (g_cl.m_flags & FL_ONGROUND) {
			...
			else if (g_cl.m_anim_time > g_cl.m_body_pred)
				g_cl.m_body_pred = g_csgo.m_globals->m_curtime + CSGO_ANIM_LOWER_REALIGN_DELAY;
		}

		m_rotation = g_cl.m_local->m_angAbsRotation();
		m_speed = g_cl.m_local->m_vecVelocity().length_2d();
		m_ground = g_cl.m_flags & FL_ONGROUND;
	}
...
void Client::OnTick(CUserCmd* cmd) {
	...
	g_lagcomp.BackupOperation(false);
	...
	// save the original state of players.
	g_lagcomp.BackupOperation(true);
...
void Client::SetAngles() {
	...
	g_cl.m_local->AddEffect(EF_NOINTERP);
...
void Client::animate() {
	...
	if (g_csgo.m_globals->m_curtime != state->m_last_update_time) {
		m_update_anims = true;
		g_csgo.m_prediction->SetLocalViewAngles(ang_t(m_real_angle.x, m_real_angle.y, m_real_angle.z));
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(Color(197, 129, 229, 255), XOR("[EssentialHook] "));
...
bool Client::CanFireWeapon(float curtime) {
	...
	// we cannot fire if the next primary attack is bigger than curtime
	return false;
}
