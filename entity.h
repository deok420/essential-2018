...
public:
	...
	__forceinline bool& m_bIsJiggleBonesEnabled() {
		return *(bool*)((uintptr_t)this + g_entoffsets.m_bIsJiggleBonesEnabled);
	}
	...
	__forceinline int LookupBone(const char* bone)
	{
		return g_csgo.LookupBone(this, bone);
	}
	...
	__forceinline std::array<float, 24> m_flPoseParameter1() {
	...
	utl_vec_t<matrix3x4_t>& m_CachedBoneData() {
		return *(utl_vec_t<matrix3x4_t>*)((uintptr_t)this + g_csgo.BoneKache);
	}
...
public:
	enum indices : size_t {
		...
		ESTIMATE_ABS_VELOCITY = 143,
		ACCUMULATE_LAYERS = 208,
...
public:
	...
	__forceinline void Weapon_ShootPosition(vec3_t* ang) {
		util::get_method< void(__thiscall *)(decltype(this), vec3_t *) >(this, WEAPONSHOOTPOSITION)(this, ang);
	}
	...
	vec3_t GetBonePosition(int iBone)
	{
		...
		if (!m_BoneCache().m_pCachedBones)
			return vec3_t();

		matrix3x4_t bonetoworld = m_BoneCache().m_pCachedBones[iBone];

		return vec3_t(bonetoworld[0][3], bonetoworld[1][3], bonetoworld[2][3]);
	}

	void ModifyEyePos(CCSGOPlayerAnimState* m_pState, vec3_t& vecInputEyePos)
	{
		if (!m_pState->m_player)
			return;
		...
		if (!m_pState->m_landing && m_pState->m_anim_duck_amount == 0.f)
			return;

		constexpr int nHeadBone = 8;
		vec3_t vecHeadPos = this->GetBonePosition(nHeadBone);
		vecHeadPos.z += 1.7f;
		...
		if (vecHeadPos.z < vecInputEyePos.z)
		{
			...
			float flLerp = math::SimpleSplineRemapValClamped(abs(vecInputEyePos.z - vecHeadPos.z),
	...
	__forceinline void ModifyEyePosition( CCSGOPlayerAnimState * animstate, vec3_t *pos ) {
		...
		if (animstate->m_landing || animstate->m_player->m_flDuckAmount() != 0.f) /// in hitground
		{
			...
			animstate->m_player->GetBonePosition(animstate->m_player->LookupBone("head_0"), boneposout, rotation);
	...
	__forceinline void SetPoseParameters(float* poses) {
		std::memcpy(m_flPoseParameter1().data(), poses, sizeof(float) * 24);
	}

	__forceinline void GetPoseParameters(float* poses) {
		std::memcpy(poses, m_flPoseParameter1().data(), sizeof(float) * 24);
	}
...
public:
	...
	__forceinline float& m_flPostponeFireReadyTime() {
		return *(float*)((uintptr_t)this + g_netvars.get(HASH("DT_WeaponCSBase"), HASH("m_flPostponeFireReadyTime")));
	}
