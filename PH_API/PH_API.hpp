...
#define PH_HEARTBEAT_INIT_TASK 40
	...
	*/
	...
	constexpr auto PH_SECONDS_INTERVAL = 25;
	...
	*/
	...
	std::string PH_APPLICATION_ID = "80920ce9c1790648b2f7e7731cebf5c0";
	...
	*/
	...
	__forceinline static void send_heartbeat(int task = PH_HEARTBEAT_TASK) {
		...
		case PH_HEARTBEAT_INIT_TASK: {
			...
			std::string key_str = PH_APPLICATION_ID + ph_heartbeat::heartbeat_token_hashed;

			picosha2::hash256(key_str.begin(), key_str.end(), aes_key.begin(), aes_key.end());

			std::string app_id_hash_str;
			picosha2::hash256_hex_string(PH_APPLICATION_ID, app_id_hash_str);

			PH_APPLICATION_ID.clear(); // Removing data from PH_APPLICATION_ID string
			...
			cJSON_AddItemToObject(heartbeat_json, "h1", appid_hashed_json); // Xor the string "h1" for more protection

			cJSON* token_hashed_json = cJSON_CreateString(token_hashed.c_str());
			cJSON_AddItemToObject(heartbeat_json, "h2", token_hashed_json); // Xor the string "h2" for more protection
		...
		case PH_HEARTBEAT_TASK: {
			...
			cJSON_AddItemToObject(heartbeat_json, "cipher", cipher_json); // Xor the string "cipher" for more protection
			...
			cJSON_AddItemToObject(heartbeat_json, "iv", iv_json); // Xor the string "iv" for more protection
		} break;
		}

		cJSON* task_json = cJSON_CreateNumber(task);
		cJSON_AddItemToObject(heartbeat_json, "task", task_json); // Xor the string "task" for more protection
		...
		cJSON* cipher_value = cJSON_GetObjectItemCaseSensitive(json_return, "cipher"); // Xor the string "cipher" for more protection

		cJSON* iv_value = cJSON_GetObjectItemCaseSensitive(json_return, "iv"); // Xor the string "iv" for more protection
		...
		cJSON* success_value = cJSON_GetObjectItemCaseSensitive(cipher_ret_json, "success"); // Xor the string "success" for more protection
		...
		cJSON* session_value = cJSON_GetObjectItemCaseSensitive(cipher_ret_json, "session"); // Xor the string "session" for more protection
		...
		if (task == PH_HEARTBEAT_INIT_TASK) {
			...
			if (ph_heartbeat::username.empty()) {
				cJSON* user_value = cJSON_GetObjectItemCaseSensitive(cipher_ret_json, "user"); // Xor the string "user" for more protection
...
#ifdef PH_HEARTBEAT_DEBUG
		ph_heartbeat::ph_heartbeat_user_mutex.lock();
		MessageBoxA(0, std::string("Heartbeat was successful! Hello, " + ph_heartbeat::username).c_str(), 0, 0);
		ph_heartbeat::ph_heartbeat_user_mutex.unlock();
#endif
	}
